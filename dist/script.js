...
const burger = (menuSelector, burgerSelector) => {
  ...
  menuElement.classList.add('animated', 'fadeInDown');
...
const filter = () => {
  ...
  const choiceType = choicedClass => {
    ...
    if (choicedClass == 'granddad' || choicedClass == 'grandmother') typeFilter();else typeFilter(choicedType);
  };
...
/* harmony import */ var _services_requests_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ../services/requests.js */ "./src/js/services/requests.js");
...
const forms = () => {
  const form = document.querySelectorAll('form'),
        ...
        finalPrice = document.querySelector('.calc-price'); // checkNumInputs('input[name="user_phone"]');
  ...
  form.forEach(item => {
    item.addEventListener('submit', e => {
      ...
      console.log(api);
      Object(_services_requests_js__WEBPACK_IMPORTED_MODULE_1__["postData"])(api, formData).then(res => {
        console.log(res);
...
const mask = selector => {
  ...
  function createMask(event) {
    ...
    } else if (event.type === 'click' && this.value.length == 2) {
...
const showMoreStyles = (trigger, wrapper) => {
  const btn = document.querySelector(trigger); // cards.forEach(item => {
